...
# Carga los paquetes de un vector de entrada y los instala si es necesario
paquetes <- function(x){
  for( i in x ){
...
# Gráficos de inspección rápida de un dataset. 
...
Vcramer<-function(v,target){
  ...
  questionr::cramer.v(table(v,target))
}
...
#Grafico con el V de cramer de todas las variables input para saber su importancia (formula de geral)
graficoVcramer<-function(matriz, target){
  salidaVcramer<-sapply(matriz,function(x) Vcramer(x,target))
  print(v<-sort(salidaVcramer,decreasing =T)) # Añadimos esto para que saque la tabla ordenada por si acaso! 
  barplot(v ,las=2,ylim=c(0,1),cex.names = 0.5)
}
